...
class TestToken {
	...
	void testException(Scanner s, String val, int riga) {
		LexicalException ex = assertThrows(LexicalException.class, s::nextToken);
		assertEquals(val + ",r:" + riga, ex.getMessage());
	...
	void testErroriID() throws IOException, LexicalException {
		...
		testException(s, "nome1", 3);

		s.nextToken();

		testException(s, "v1r", 5);
	...
	void testIntErrori() throws FileNotFoundException {
		...
		testException(s, "123r", 3);

		testException(s, "12s3", 4);

		testException(s, "7y", 5);

		testException(s, "00", 7);

		testException(s, "00p", 7);

		testException(s, "00p0", 8);

		testException(s, "070", 9);

		testException(s, "07p", 10);

		testException(s, "000", 11);
	}
	...
	void testErroriNumbers() throws FileNotFoundException {
		...
		testException(s, "00", 1);

		testException(s, "123a", 2);

		testException(s, "12.a", 3);

		testException(s, "123.121212", 4);
	}
